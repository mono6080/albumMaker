...
搬動的東西：
    ProjectStudent row（pages_data_json 裡的個人文字與跳過設定跟著 row 走）
...
from database import ClassroomMember, ProjectStudent
        ...
        if child_ids:
            active_child_ids = {
                ...
                for (child_id,) in db.query(ClassroomMember.roster_child_id).filter(
                    ClassroomMember.classroom_id == source.classroom_id,
                    ClassroomMember.roster_child_id.in_(child_ids),
