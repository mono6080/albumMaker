...
def _add_legacy_project_identity_migration_schema(connection):
    ...
    # 先 DROP 再建（不能用 IF NOT EXISTS）：舊資料庫裡已經有一份「一律禁止改
    # project_id」的定義，要被下面帶窄口的版本取代掉。
    connection.execute(text(
        "DROP TRIGGER IF EXISTS trg_project_students_freeze_class_backed_identity"
    ))
    connection.execute(text(f"""
        CREATE TRIGGER trg_project_students_freeze_class_backed_identity
