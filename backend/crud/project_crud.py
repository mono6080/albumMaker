# 專案與學生 CRUD 輔助模組
# 封裝對 Project / ProjectStudent 的常用資料庫查詢，
...
def get_student_or_404(student_id: int, project_id: int, db: Session) -> ProjectStudent:
    """查詢指定學生，若不存在或不屬於該專案則拋出 404。"""
    student = db.query(ProjectStudent).filter(
        ProjectStudent.id == student_id,
