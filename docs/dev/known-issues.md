...
- **期中轉班可能讓同一期出現兩本相本，而且解不掉**：孩子在原班已建立該期相本之後
  才轉班，新班之後又建同一期相本時，他在兩本裡都有 `ProjectStudent` 快照。彙整匯出
  會把那一格判為 `duplicate`：不渲染、不納入 ZIP，只在匯出說明列出「Project X, Y
  同一期有重複相本，未納入」。
  現行**沒有出口**——兩本相本通常都還有其他孩子所以不能封存，而學生集合由
  `trg_project_students_freeze_class_backed_identity` 凍結，沒有從相本移除單一學生的
  入口。實務上只能在轉班前先確認原班那一期的相本是否已經建立。
  非學期範圍班級重構引入，master 即是此行為；2026-08-01 實測確認轉班後兩本相本
  各自保留該孩子，該期 cell 狀態為 `duplicate`。
