...
11. **正式站已存在一個 `status='cancelled'` 的「115上」學期**（id=2，2026-08-01 ~
   ...
   同名學期
12. **115 上的模板要搬到正式站**。模板一律在本機匯入（`import_office_template.py` 需要
   Windows + Word COM，容器裡跑不動），再用
   `scripts/transfer_template.py` ＋ `scripts/migrate_uploads_to_r2.py` 搬過去；
   步驟見 [storage.md](../dev/storage.md#把本機匯入好的模板搬到正式站)。
   **要先建好目標期別**，否則會被擋下。

   沒搬模板不會讓系統壞掉，但老師在 115 上建不出相本——可以排在編班之後，不必卡上線。
   2026-08-04 已在正式資料副本上實測 7 個模板（12階／3階／4階／5階／6階／7階／銜接
   的主題一）：路徑全部對得上、原有 23 個模板未動、七頁算圖正常。

13. **編班時新生要連學號一起輸入**（格式「姓名 學號」，可從行政系統的表格整欄複製）。
