...
編班頁的學生編排改成**一欄一個新學期班級的看板**，班級的所有操作都在同一個畫面完成。
原本「逐班展開 + 每位學生一個下拉」的介面已移除，「可編入的目標班級」清單與
「新學期老師編制」卡片牆兩個區塊也一併刪除——它們是同一份資料的第二、第三份列表。

- **分頁依分校**：38 欄一次攤開要橫向捲 7400px。分校是天然的分段，因為班級的分校
  不可變更。「離園」是每一頁的最後一欄，且只列**該校來源**的學生——離園的孩子沒有
  目標班級，只能用來源分校歸屬。
- **選取跨分頁保留**，所以跨校搬動的路徑是「這邊選、切分頁、點目標班標題」。
  拖曳只在同一分頁內成立。
- **多選**：點卡片逐個切換，或在空白處拖出框一次框選（Shift 為加選）。同一次只能
  選同一類——學生與老師搬去別班的語意不同，混在一起沒有正確解。
- **老師**顯示在班名下方，可拖到別班；落到沒有主教的班會成為主教，否則成為協同。
  點「協」是**換人當主教**（原主教同時降為協同），因為非空編制恰有一位主教是驗證要求，
  也有 DB 唯一鍵擋著。
- **班級順序**由 `Classroom.sort_order` 決定，只能在同校內調整，見
  [data-model.md](../dev/data-model.md#班級的顯示順序)。
- **改名與調順序不重載頁面**：`loadWorkspace()` 會把伺服器版本蓋回本地並清掉
  `isDirty`，未儲存的學生與老師調整會無聲消失。這兩件事不影響草稿內容，本地更新即可。
  增班／減班會改動草稿的班級集合、之後必須重載，所以在未儲存時直接擋下。
- **新增到草稿學期的班會自動補一列 `TermClassroomPlan`**，否則它不會出現在老師編制裡，
  套用後成為無人帶班的空班——而且整個過程不會報錯。

## Open Questions

- ~~`students.student_serial`（行政系統學號）要在本次一併加入，還是等結構穩定後獨立一刀？~~
  **已決（2026-08-01）：本次加入。** 匯入 115上 編班時實測，用學號對 402 位在籍學生
  有 378 位對得到、24 位是真新生，零灰色地帶；改用姓名對同一批資料則多出 28 個假新生。
  姓名不是可靠的鍵——這次才更正過 31 筆。落點見
  [data-model.md](../dev/data-model.md#名冊與行政系統的對帳鍵)。
