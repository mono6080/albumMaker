...
`全期合併_孩子.pdf`，頁序跨期連續；僅一期時不附。

版式由匯出者在下載當下選 `sheet_layout`（`spread` 雙頁 A3 為預設、`single` 單頁 A4），
**套用到這包裡的每一份 PDF**——每期檔與全期合併檔一致，期別數不影響版式：

- `spread`：以 pypdf 把 A4 頁兩頁併成一張 A3 橫式，頁數為奇數時最後一張右半留白。
  單期的孩子同樣拿到 A3，不因為沒有合併檔而落回 A4。
- `single`：維持相本原頁；合併檔只是依期別順序串接，每期檔沿用原始 bytes 不重寫。

全期合併檔一律從各期原始 A4 頁重組，不是把已併好的 A3 再接起來，
所以 `spread` 下跨期的併頁邊界與各期檔的併頁邊界可以不同。
