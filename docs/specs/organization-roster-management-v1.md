...
但班級相本的老師**製作權**直接來自目前 `ClassroomTeacher`。調班後新老師取得
該班各期相本工作權、舊老師失去製作權；要取得製作權就必須仍在班級編制，不能靠 editor
旁路。

Decision：**讀**與**製作**分離——老師讀得到目前任教班級與**曾任教班級**（`ended_at`
不為空的編制）的相本，製作權只跟著目前編制。理由是相本是老師自己做出來的成果，學期
轉換不該讓人看不到自己去年的作品；若讀寫綁在一起，就必須讓「班級」在學期之間人工維
持延續關係（把八階A 改名成九階A 讓老師留在同一筆班級上），而分班本來就會拆散混齡的
銜接班，那個延續關係無法自動推導。曾任教班級不計入 `classroom_ids`，`my-classrooms`
與建立相本的閘門仍只看目前編制。曾任教班級不篩 `Classroom.is_active`：班級停用是園所
的組織異動，不該讓老師失去自己做過的相本。
...
Decision：舊相本的 ProjectStudent 身分不按姓名自動遷移。未歸班 Project 的既有
`roster_child_id` 只算 provisional evidence；管理員必須先預覽，再為每位 ProjectStudent 明確選擇
...
- 相本的成員與 `ProjectStudent.name` 是建立當下班級名單快照；相本稱呼只讀園所設定，老師不可
...
- 不在編班套用時自動建立 Project，也不重寫既有 Project、ProjectStudent 或 owner。
...
`ProjectStudent.album_name` 不再是資料來源；API、預覽與渲染都動態讀取 `Student.album_name`，
未設定時回退該本 `ProjectStudent.name` 快照。園所修改中央稱呼會失效所有相關既有輸出，但不改
成員、完整姓名或 child link。`ProjectStudent.album_name` 只保留給未歸班 legacy Project。

同一個班級期別現在可以有多本相本（見
[學期報表 spec](academic-term-reporting-v1.md)）。凍結因此開一個窄口：**同一個
`class_period_work_slot_id`、同一個 `template_id` 的兩本之間可以搬移 ProjectStudent**
（`POST /projects/{id}/students/transfer`）。這不踩到凍結要防的三件事——孩子仍在同班
同期、身分與姓名完全沒動、期末彙整仍只出現一次。DB trigger
`trg_project_students_freeze_class_backed_identity` 只在這個條件下放行 `project_id`，
`name` 與 `roster_child_id` 維持完全不可變。搬移會一併搬照片實體與頁面資料、清除
來源的該生輸出與個別完成狀態；來源不可被搬空，兩本都必須未完成。

凍結另開第二個窄口：**名冊上還沒被這一格任何相本收錄的孩子可以補進既有相本**
（`POST /projects/{id}/students/from-roster`）。期中入學的孩子是相本開好之後才進名冊的，
成員集合若完全凍結，他只能自己獨立一本——期末彙整因此多一本、老師也得為一個孩子重做
一次版面。這個窄口補的正是凍結要防的第三件事「漏算」：孩子仍在同班同期、身分直接取自
目前名單、同格沒有別本收過他。實作只 INSERT `ProjectStudent`（`pages_data_json="[]"`、
`order_index` 接續），既有成員一列都不動，因此 trigger 擋的 UPDATE 完全沒被觸及。限制：
相本須已歸班、孩子須在該班目前名單（carryover）、同格未收錄過、不超過單本人數上限、
同本不得同名；已標記完成的相本由內容鎖擋下，非 admin 不得補。期別建立鎖不參與判斷——
那個鎖管的是「還能不能開新相本」，見
...
`Student` 與第一段 `ClassroomMember`；即使姓名相同也不共用 id。改名只更新這個
目前名冊身分；可選 `Student.album_name` 是所有已歸班相本唯一稱呼來源。轉班、回班與
新學期重新編班都沿用原 `Student.id`，因此稱呼自然沿用，只改 membership 區間。
新入園若未明確提供稱呼，加入名單時才保守自動推導一次。由目前名單建立 Project 時，
ProjectStudent 沿用 child id、固定當下完整姓名且不複製稱呼。Project runtime 不得改
`ProjectStudent.roster_child_id`、`ProjectStudent.name` 或 class-backed `ProjectStudent.album_name`。

fresh legacy schema 加入 `project_students.roster_child_id` 時一律保持 `NULL`，不得按姓名建立或共用
`Student`。曾跑過舊姓名推定 migration 的資料不在 startup 自動拆分；只要 Project 尚未
歸班，那個 link 就只是待人工判讀的 provisional evidence，不是 authority。學期匯出與所有
老師／主管報表完全排除未歸班 Project，因此 provisional link 不得參與分組。

established identity 只包括已有任一 `ClassroomMember`，或已被 class-backed Project
ProjectStudent 使用的 `Student`。舊相本歸班的 `existing` 決策只能指向 established identity；
`create_new` 永遠建立全新 `Student`，不得沿用或 promote provisional id。成功歸班後
ProjectStudent identity 由 service 與 DB trigger 共同凍結；沒有普通 runtime link、merge 或 split API。
...
`(id,campus_id,department,name)`，active membership id、`Student` id／正規化姓名／
...
`term_reassignment`。apply 不得呼叫任何修改既有 Project／ProjectStudent／ACL 的 service。
...
v1 新入園學生仍走園所設定的「加入目前名單」流程，在加入時建立新的 `Student`；
編班草稿只移動既有身分，不建立學生身分；
...
- `POST /api/organization/classrooms/{id}/members/batch`：admin 新增入園學生；每一筆建立新的
  `Student` 與 active membership，不按姓名重用既有身分；可選中央 `album_name`，
  省略時套用保守推導。
- `PATCH /api/organization/classrooms/{id}/members/{member_id}`：admin 改名、設定或清除中央
  `album_name`、離園、轉班或回班；轉班／回班沿用原 `Student.id`，只新增
  不可覆寫的 membership 區間。
- `PATCH /api/organization/students/{id}/album-name`：admin 修改沒有可用 membership
...
主管報表也一律排除未歸班 Project，即使其 ProjectStudent 尚帶有非 NULL provisional link。
...
- 移除老師：立即失去該班授權；既有 Project、ProjectStudent 與 owner 歸戶不改。
...
- fresh legacy ProjectStudent 的 `roster_child_id` 保持 `NULL`；already-migrated 姓名推定 link 保留為
  provisional evidence，startup 不自動拆分、合併或視為已解析。
- startup 若發現任何 class-backed Project 含 NULL link、失效 link，或同一 Project 多位
  ProjectStudent 共用 child id，就把整本 Project（含已封存者）的 `classroom_id` 清為 NULL，重新
  隔離於 admin-only 歸班佇列。不得猜測或改寫 ProjectStudent identity，也不得清除 Project 校／班
...
- apply 共用 `organization_acl_lock` 與 `BEGIN IMMEDIATE`：重讀 Project／班級／ProjectStudent，驗證
  department、`confirmed_all`、重算 `source_fingerprint`、完整 decisions、identity 唯一性、
  seed 衝突與容量；接著寫入 raw audit ledger、
  更新未歸班 ProjectStudent link、必要時建立全體 membership，最後才寫 Project `classroom_id` 與
  校／班快照並一次 commit。任一步失敗時 ledger、Student、ProjectStudent、membership、Project
...
8. fresh legacy ProjectStudent link 保持 NULL；舊姓名推定 link 只作 provisional evidence。未歸班相本
   對非 admin 為 403 且不進匯出；preview 不預填決策，apply 缺任一 ProjectStudent decision 就零寫入。
9. admin 對一個空班完整選擇 create-new／established-existing 並全量 seed；ledger、ProjectStudent links、
...
12. 重新編班不改舊 Project、ProjectStudent、owner 與班級名稱快照；新老師立即取得目標班全部
