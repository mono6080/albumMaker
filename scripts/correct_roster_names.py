# 已退場：2026-08-01 已在正式站執行完畢（姓名 31、快照 123、稱呼 28、退回 11 本）。
# 且本腳本寫死 `roster_children`／`students` 舊表名，班級改為學期範圍實體後表已改名，
# 對現行資料庫執行只會找不到表。保留僅供稽核，不可執行。
...
可重複執行：每一步都以「現況與目標的差異」為準。

**但必須跑兩遍才會收斂。** 第 3 步的判斷條件是「稱呼不再是姓名的一部分」，姓名還沒
改進去之前這個條件不成立，所以第一遍必定回報稱呼 0 位——要等第 1 步把姓名寫進去，
第二遍才看得到那些從舊姓名推導出來的錯稱呼。第三遍才會全 0。
只跑一遍不會有任何錯誤訊息，但相本會印著錯的稱呼。
