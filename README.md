...
設計組在線上設計模板（版型、照片格、氣泡框、貼圖），老師批次匯入學生名單、
上傳照片、填寫對應文字後一鍵產生 PDF，並可下載全班壓縮包；主管與美學組
可在審閱頁留言回饋。
...
| 模組 | 功能 |
|------|------|
| **使用者系統** | JWT 登入、5 種角色（管理員/美學組/主管/帶班老師/無權限）、多主管指派、.xlsx 批次匯入 |
| **模板系統** | 期別分組管理；多頁版型編輯器（Konva 畫布）：照片格、氣泡框、文字標籤、貼圖、背景圖、跨頁預覽 |
| **批次管理** | 貼上名單批次新增學生；全班統一對應文字；共用照片與依檔名批次分配 |
| **個別編輯** | 單一學生照片上傳、位移縮放裁切、個人化文字覆蓋、頁面跳過 |
| **輸出審閱** | 頁面預覽、留言回饋、PDF / 圖片 ZIP 下載（完整畫質僅限管理員）、專案封存 30 天 |
...
| 資料模型與版型格式 | [docs/dev/data-model.md](docs/dev/data-model.md) |
