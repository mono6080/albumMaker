...
設計組在線上設計模板（版型、照片格、氣泡框、貼圖），老師在**班級總覽**
（工作台）管理學生名單、追蹤照片進度，在**相本編輯器**以「全班／個別」
...
| **班級總覽（工作台）** | 學生名單管理（批次貼上/改名/刪除）、照片進度與階段引導（製作→全班完成→交件）、審閱意見、單人與全班 PDF / 圖片 ZIP 下載 |
| **相本編輯器** | 全班／個別兩種編輯範圍一鍵切換：全班共用照片（選格→每人不同張/全班同一張/依檔名整批匯入）與共用文字；個別學生照片上傳、位移縮放裁切、文字覆寫、頁面跳過 |
| **完成與交件** | 全班完成鎖定（主管/管理員可退回）、完整畫質下載僅限管理員、專案封存 30 天 |
